#!/usr/bin/env python3
"""Plot OBJ torque-align campaign: orientation vs tidal spin change."""
...
def delta_percent(new: np.ndarray, base: np.ndarray) -> np.ndarray:
...
def resolve_batch_csv(repo: Path, explicit: Path | None) -> Path:
    ...
    matches = sorted(
        (repo / "sobol_mass_runs").glob(
            "sobol_*_flyby_spin_torque_align_obj_k1e7/sobol_mass_outputs.csv"
        )
    )
    if not matches:
        raise SystemExit(
            "No flyby_spin_torque_align_obj_k1e7 batch found under sobol_mass_runs/"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        default=Path("sobol_mass_runs/plots/flyby_spin_torque_align_obj_k1e7.png"),
    )
    ...
    d_post_app = delta_percent(d["post"], d["app"])
    d_post_intr = delta_percent(d["post"], d["intr"])
    d_app_intr = delta_percent(d["app"], d["intr"])
    ...
    ax_sanity2.set_ylim(0, max(5.0, float(np.max(d["unb"])) * 100 * 1.2))
    ...
    fig.suptitle(
        "OBJ DEM: torque-align vs post-flyby spin period change\n"
        r"$P = 2.05$ hr, $k_c = 10^7$ dyne/cm, $n_p = 500$, CA $\approx$ 38,000 km",
        fontsize=11,
    )
    fig.tight_layout()
