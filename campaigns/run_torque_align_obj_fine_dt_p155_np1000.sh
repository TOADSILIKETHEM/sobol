#!/usr/bin/env bash
# OBJ-cropped opposite_near_h breakup re-run: spin 1.55 hr, np=1000, 5-min dumps.
...
LOG="${LOG:-sobol_mass_runs/torque_align_obj_fine_dt_p155_np1000_stdout.log}"
...
python3 sobol/run_torque_align_blender_reruns.py \
  ...
  --batch-label torque_align_obj_fine_dt_p155_np1000 \
  "$@" 2>&1 | tee -a "$LOG"
