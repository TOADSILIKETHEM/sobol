#!/usr/bin/env bash
# Flyby DEM: fixed k_c and spin period; sweep spin alignment vs flyby orbit angular momentum.
# See campaigns/flyby_spin_torque_align_k1e7.md
set -euo pipefail
...
LOG="${LOG:-sobol_mass_runs/flyby_spin_torque_align_k1e7_stdout.log}"
mkdir -p sobol_mass_runs

echo "[INFO] Starting flyby_spin_torque_align_k1e7 at $(date -Is)" | tee -a "$LOG"
...
python3 sobol/run_mass_sobol_phantom.py \
  ...
  --num-samples 24 \
  --seed 43 \
  --batch-label flyby_spin_torque_align_k1e7 \
  --use-dem-fixed true \
  --np-apophis 500 \
  --kc-fixed 1e7 \
